...
function getPrevPeriodBounds(f: Filters) {
  const { from, to } = getDateBounds(f);
...
function detectEmailType(subject: string): string {
  ...
  if (lower.includes("trigger") || lower.includes("growth") || lower.includes("funding") || lower.includes("hiring")) return "trigger_based";
  return "other";
}
...
export interface EmailTypeMetric { type: string; label: string; sent: number; opens: number; clicks: number; openRate: number; clickRate: number }
...
async function fetchPeriodMetrics(from: string, to: string) {
...
export function useOutreachAnalytics(filters: Filters) {
  ...
  const prev = getPrevPeriodBounds(filters);
  ...
  const overall = useQuery({
    ...
    queryFn: async (): Promise<OverallMetrics> => {
      const [current, previous] = await Promise.all([
        ...
        fetchPeriodMetrics(prev.from, prev.to),
      ]);
      return {
        ...current,
        prevTotalSent: previous.totalSent,
        prevOpenRate: previous.openRate,
        prevClickRate: previous.clickRate,
        prevReplyRate: previous.replyRate,
        prevClickToOpenRate: previous.clickToOpenRate,
      };
    },
  });
  ...
  const competitorPerformance = useQuery({
    ...
    queryFn: async (): Promise<CompetitorMetric[]> => {
      const { data: emails } = await supabase
        .from("email_send_log")
        .select("message_id, subject, created_at, metadata")
        .eq("status", "sent")
        .gte("created_at", from)
        .lte("created_at", to);
      if (!emails?.length) return [];
      ...
      for (const e of emails) {
        // Module 3: Metadata-first competitor detection
        const meta = e.metadata as Record<string, unknown> | null;
        const metaAngle = meta?.competitor_angle as string | undefined;
        const comp = metaAngle ?? queueMap.get(e.subject ?? "") ?? detectCompetitor(e.subject ?? "") ?? "No Angle";
  ...
  const emailTypePerformance = useQuery({
    queryKey: ["outreach-analytics-emailtype", from, to],
    queryFn: async (): Promise<EmailTypeMetric[]> => {
      const { data: emails } = await supabase
        .from("email_send_log")
        .select("message_id, subject, metadata")
        .eq("status", "sent")
        .gte("created_at", from)
        .lte("created_at", to);
      if (!emails?.length) return [];
      ...
      const typeMap = new Map<string, { sent: number; opens: number; clicks: number }>();
      for (const e of emails) {
        const meta = e.metadata as Record<string, unknown> | null;
        const metaType = meta?.email_type as string | undefined;
        const type = metaType ?? detectEmailType(e.subject ?? "");
        if (!typeMap.has(type)) typeMap.set(type, { sent: 0, opens: 0, clicks: 0 });
        typeMap.get(type)!.sent++;
        if (e.message_id && openSet.has(e.message_id)) typeMap.get(type)!.opens++;
        if (e.message_id && clickSet.has(e.message_id)) typeMap.get(type)!.clicks++;
      }
      return [...typeMap.entries()]
        .map(([type, d]) => ({
          type,
          label: EMAIL_TYPE_LABELS[type] ?? type.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase()),
          sent: d.sent,
          opens: d.opens,
          clicks: d.clicks,
          openRate: d.sent > 0 ? Math.round((d.opens / d.sent) * 100) : 0,
  ...
  return { overall, timeSeries, competitorPerformance, subjectLeaderboard, sendTimeHeatmap, emailTypePerformance };
}
...
export function generateInsights(
  ...
  if (metrics.prevTotalSent > 0) {
    const delta = metrics.openRate - metrics.prevOpenRate;
    if (delta !== 0) {
      insights.push(`Open rate ${delta > 0 ? "increased" : "decreased"} by ${Math.abs(delta)}% compared to the previous period.`);
    }
  }
